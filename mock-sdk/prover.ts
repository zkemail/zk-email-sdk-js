import { ProverParams, ProofParams } from '../prover';
...
export class ProverMock {
   ...
   */
  generateProof(input: string | Buffer, blueprintId: string): ProofMock {
    const proofRequest: Promise<Buffer> = new Promise(async (resolve) => {
      await new Promise((r) => setTimeout(r, 3_000));
      resolve(Buffer.from('proof data'));
    });

    return new ProofMock({
      isLocal: false,
      blueprintId,
