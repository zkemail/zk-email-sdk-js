...
import { NOIR_V2_FIXED_OUTPUTS } from "./prover/noir";
  ...
  try {
    ...
    if (proof.props.zkFramework === ZkFramework.Noir) {
      ...
      if (!Array.isArray(outputs)) {
        throw new Error(
          `Noir proof has an unexpected publicOutputs shape (expected string[], got ${typeof outputs}). Verify the proof was generated by NoirProver and that the zkFramework field is set correctly.`
        );
      }
      if (outputs.length < NOIR_V2_FIXED_OUTPUTS) {
        throw new Error(
          `This Noir proof was generated against a pre-v2.0.0 zkemail.nr circuit and can no longer be verified securely (REG-670). The circuit did not commit to the Barrett reduction parameters, so the public-key check can be bypassed. Recompile the blueprint against zkemail.nr v2.0.0+ and regenerate the proof. (Found ${outputs.length} public outputs; v2.0.0+ requires at least ${NOIR_V2_FIXED_OUTPUTS}.)`
        );
      }
      const redcHashRaw = outputs[1];
      if (!redcHashRaw || redcHashRaw === "0x") {
        throw new Error(
          "Noir proof publicOutputs[1] (redc hash) is empty or malformed. Expected zkemail.nr v2.0.0+ output layout — regenerate the proof against a v2.0.0+ circuit."
        );
      }
      try {
        redcHash = BigInt(redcHashRaw).toString();
      } catch (parseErr) {
        throw new Error(
          `Noir proof publicOutputs[1] (redc hash) is not a valid hex/decimal value: ${redcHashRaw}. Expected zkemail.nr v2.0.0+ output layout — regenerate the proof against a v2.0.0+ circuit. (REG-670)`
        );
      }
    }
